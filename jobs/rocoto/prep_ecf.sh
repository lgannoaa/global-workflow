...
#### This break ecflow workflow DATA assignment must be removed
. $HOMEgfs/ush/load_fv3gfs_modules_ecflow.sh
status=$?
...
#module use $HOMEprepobs/modulefiles/prepobs
#### Workaround for prepobs_run_ver inconsist with module location
#### module load prepobs/$prepobs_run_ver
#module load prepobs/$prepobs_module_ver
module list
###############################################################
...
export COMOUT="$COMOUT_PREP/$CDUMP.$PDY/$cyc/$COMPONENT"
...
####   gCOMOUT="$ROTDIR/$GDUMP.$gPDY/$gcyc/$COMPONENT"
   gCOMOUT="$COMOUT_PREP/$GDUMP.$gPDY/$gcyc/$COMPONENT"
...
#fi
cp $DMPDIR/${CDUMP}${DUMP_SUFFIX}.${PDY}/${cyc}/${COMPONENT}/${CDUMP}.t${cyc}z.syndata.tcvitals.tm00 $ROTDIR/$CDUMP.$PDY/$cyc/$COMPONENT
...
if [ $DO_MAKEPREPBUFR = "YES" ]; then
    ...
    export DATAROOT=${DATAROOT:-$RUNDIR/$CDUMP/prepbufr}
...
####      export COMSP=${COMSP:-$ROTDIR/${CDUMP}.${PDY}/${cyc}/$COMPONENT/$CDUMP.t${cyc}z.}
      export COMSP=$COMOUT/$CDUMP.t${cyc}z.
#    fi
