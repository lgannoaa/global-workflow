...
### Feb 2017: Updated test for BIAS to account for sign changes by Fanglin Yang. 
...
#export webhostid=${webhostid:-"wx24fy"}               ;#login id on rzdm webhost
...
export caplist=${caplist:-$mdlist}                     ;#caption list 
...
export day=${day4card:-"1 3 5 6 8 10"}
ndd=`echo $day |wc -w`
...
if [[ $reg1 = "G2" ]] ; then
   world=$(echo "<th colspan=$ndd >Globe</th>")
elif [[ $reg1 = "G2NHX" ]] ; then
   world=$(echo "<th colspan=$ndd >N. Hemisphere</th>")
elif [[ $reg1 = "G2SHX" ]] ; then
   world=$(echo "<th colspan=$ndd >S. Hemisphere</th>")
elif [[ $reg1 = "G2TRO" ]] ; then
   world=$(echo "<th colspan=$ndd >Tropics</th>") 
elif [[ $reg1 = "G2PNA" ]] ; then
   world=$(echo "<th colspan=$ndd >N. American</th>")
fi
